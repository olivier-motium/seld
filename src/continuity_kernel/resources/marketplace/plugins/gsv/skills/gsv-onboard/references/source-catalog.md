...
| Discord | Seld's read-only `discord-mcp` companion (local MCP server, token via environment) | Confirm the configured account identity with `discord_source_status`; create a forward-only baseline on the first bounded `discord_poll_messages` read, persist the source record, then advance the cursor with `discord_acknowledge_messages`. Never send, react, or expose the token. |

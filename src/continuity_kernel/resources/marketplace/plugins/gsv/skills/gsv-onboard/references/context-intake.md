...
Use plain language and adapt the wording to the conversation:
...
private message bodies. Authentication belongs in the provider's own surface.

## Mirror the proposed context

Return a compact intake card containing:

- the desired outcome;
- each proposed life or work context, with **accept**, **edit**, or **reject**;
- the first proof;
- sources grouped as **now**, **later**, or **off**;
- exclusions and retention limits;
- the external-action boundary; and
- uncertainties that still need confirmation.

Only an accepted or corrected statement is eligible to become durable context.
Persist it only through a supported canonical mutation surface. Preserve the
difference between a user statement, an observed fact, and an inference. Never
turn research, a question, or a model guess into memory automatically.

When a supported canonical mutation surface exists, store the smallest useful

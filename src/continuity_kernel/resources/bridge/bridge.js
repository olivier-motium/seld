...
function renderContinuity(snapshot) {
  ...
  line.append(
    ...
    continuityStage("02", "Hands", inMotion ? `${inMotion} in motion` : "Replaceable"),
...
function continuityStage(index, label, value, className = "") {
  const item = element("div", `continuity-stage ${className}`.trim());
  item.append(
    textElement("span", "continuity-index", index),
...
function setConnectionState(connectionState, message = "") {
  ...
  ui.localStatus.querySelector("span:last-child").textContent = labels[connectionState];
  if (partial) {
